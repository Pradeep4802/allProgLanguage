...
function readData() {
  // this will help in reading the form data
  var formdata = {};
...
function insertData(data) {
  // this will help in inserting the data in table
...
function validateAge(value) {
  if (value == "") {
    ...
    if ((a >= 15 && a <= 35) || (a >= 45 && a <= 65)) {
      ...
      document.getElementById("isAgeValid").innerHTML =
        "age must be within 15 to 35 and 45 to 65";
      document.getElementById("isAgeValid_edit").innerHTML =
        "age must be within 15 to 35 and 45 to 65";
      return false;
    }
...
function onEdit(value) {
  ...
  idx = row.rowIndex - 1;
  document.getElementById("edit_id1").value =
    table.rows[idx].cells[0].innerText;
  document.getElementById("edit_name").value =
    table.rows[idx].cells[1].innerText;
  document.getElementById("edit_age").value =
    table.rows[idx].cells[2].innerText;
  document.getElementById("edit_image").value =
    table.rows[idx].cells[3].innerText;
  document.getElementById("edit_gender").value =
    table.rows[idx].cells[4].innerText;
  document.getElementById("edit_designation").value =
    table.rows[idx].cells[5].innerText;

  console.log("idx after= " + idx);
}

function update() {
  //once edited data is updated here
  event.preventDefault();
...
function onView(target) {
  ...
  document.getElementById("view_id1").value =
    table.rows[cellindx].cells[0].innerText;
  document.getElementById("view_name").value =
    table.rows[cellindx].cells[1].innerText;
  document.getElementById("view_age").value =
    table.rows[cellindx].cells[2].innerText;
  document.getElementById("selfImage").src =
    table.rows[cellindx].cells[3].innerText;

  document.getElementById("view_gender").value =
    table.rows[cellindx].cells[4].innerText;
  document.getElementById("view_designation").value =
    table.rows[cellindx].cells[5].innerText;
